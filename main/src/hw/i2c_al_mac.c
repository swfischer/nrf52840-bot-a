/*****************************************************************************
 * i2c_al_mac.c - An abstraction layer (al) interface to an I2C bus.
 * 
...
static i2c_al_data i2c_al_hndl_data[MAX_NUMBER_OF_HANDLES] = {0};
...
int i2c_al_init(const i2c_al_device_t *device)
{
    ...
    } else if (i2c_al_hndl_data[tmp_hndl].initialized != false) {
        LOG_INF("Already initialized");
    } else { 
        i2c_al_data *fd_data = &i2c_al_hndl_data[tmp_hndl];
...
void i2c_al_exit(int hndl)
{
    ...
    } else if (i2c_al_hndl_data[hndl].initialized == true) {
        i2c_al_hndl_data[hndl].initialized = false;
    }
}
...
int i2c_al_read(int hndl, uint8_t *rbuf, uint16_t rsize, const uint8_t *wbuf, uint16_t wsize)
{
    ...
    if ((hndl < 0) || (hndl >= MAX_NUMBER_OF_HANDLES)) {
        LOG_WRN("Invalid hndl");
    } else if (i2c_al_hndl_data[hndl].initialized == false) {
        LOG_WRN("Un-init'd hndl");
    } else if ((rbuf == NULL) || (rsize == 0U)) {
        // Just return an error
    } else {
        //i2c_al_data *fd_data = &i2c_al_hndl_data[hndl];

        int err = -1;

        if ((wbuf == NULL) || (wsize == 0U)) {
            err = -1; //i2c_read(fd_data->i2c_dev, rbuf, rsize, fd_data->i2c_addr);
        } else {
            err = -1; //i2c_write_read(fd_data->i2c_dev, fd_data->i2c_addr, wbuf, wsize, rbuf, rsize);
        }
...
int i2c_al_write(int hndl, const uint8_t *wbuf, uint16_t wsize)
{
    ...
    if ((hndl < 0) || (hndl >= MAX_NUMBER_OF_HANDLES)) {
        ...
        int err = -1; //i2c_write(fd_data->i2c_dev, wbuf, wsize, fd_data->i2c_addr);
        if (err != 0) {
